---
title: "M3 · 张量并行 TP + SP"
description: "FSDP 解决「模型状态常驻不下」，TP 解决「单层计算与中间激活放不下」。代价是：Transformer 每一层都要在前向/反向的关键路径上同步。先看为什么，再从一个矩阵推到完整 Transformer layer。"
type: series
status: stable
level: intermediate
updated: 2026-08-24
...
# 张量并行 TP：切开一层，而不是临时拼回一层

<div class="notebook-hero" markdown>

<span class="chapter-kicker">Module 3 · Tensor Parallelism</span>

FSDP 解决「模型状态常驻不下」，TP 解决「单层计算与中间激活放不下」。代价是：Transformer 每一层都要在前向/反向的关键路径上同步。先看为什么，再从一个矩阵推到完整 Transformer layer。
...
纯 TP 只切线性层内部。每次 Row Parallel 的 all-reduce 之后，hidden states 又在所有 TP rank 上复制；RMSNorm/LayerNorm、Dropout、残差分支也因此各保存一份 $[S,B,H]$ 激活。Sequence Parallel（SP）把这些**不需要跨 token 交互**的区域按序列维 $S$ 切成 $[S/T,B,H]$。
...
!!! note "SP 与 CP 不同"
    ...
    SP 只分摊逐 token 区域的激活存储，计算 attention 前会在 TP 组内还原序列；Context Parallel 则让 attention 本身也在分段序列上计算。SP 通常随 TP 一起开启，TP=1 时没有切分收益。
...
## 06 · 常见 Transformer Layer 怎么切？到底有几次通信 { #layer }
