...
1. **复制**：每张卡持有一份完整、相同的模型；在经典 DDP 中，参数、梯度和优化器状态都不沿数据并行维分片。
2. **切数据**：把一个 global batch 沿样本维分成 $N$ 份 local batch，每个 data-parallel rank 独立完成前向和反向。这里的 local batch 还可以继续拆成多个 micro-batch 做梯度累积，不要把两个概念混为一谈。
3. **同步梯度**：各 rank 算出的局部梯度不同，在优化器更新前用 `all-reduce` 对各梯度 bucket 求和或平均。所有 bucket 完成后，每个 rank 都拥有同一份完整梯度，因而更新后的模型继续一致。
...
本节以 Megatron-LM 的经典 DDP 路径为具体实现。它的关键不只在于 all-reduce 梯度，还在于如何以 bucket 为单位把通信插入反向过程。下面沿梯度的生命周期看三个设计。


### ① 连续梯度 buffer + bucket 分桶

所有参数的梯度不是各自为政，而是预分配进**一整块连续显存**，每个 `param.main_grad` 指向其中一段切片。在本文采用的 Megatron BF16 路径中，这个累积与通信 buffer 使用 FP32，因此它虽然只有 $P$ 个元素，却占 $4P$ 字节。再把参数按**反向传播的逆序**切成若干 bucket：
...
## 03 · 经典 DP 的单卡显存 { #nomem }

这是 DP 最重要的局限，也是后面所有「切模型状态」并行存在的理由。沿用第 1 章的 Megatron BF16 + Adam 账本假设，纯 DDP 的每个 rank 都持有完整的 BF16 参数（$2P$）、FP32 `main_grad`（$4P$）与优化器状态（$12P$）。all-reduce 只改变梯度数值，不改变这些对象按完整模型分配的容量：
...
!!! warning "⚠️ 结论"
    ...
    在模型、local batch、精度和激活策略不变时，经典 DDP 的模型状态仍约为每卡 $18P$ 字节，不随 DP rank 数下降；激活显存则由每卡实际处理的 local batch 等因素决定。DP 切的是*样本及其计算*，不切*模型状态*。下一章将沿数据并行维进一步切分优化器状态、梯度和参数。
